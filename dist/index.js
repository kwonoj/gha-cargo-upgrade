...
// node_modules/octokit-plugin-create-pull-request/dist-node/index.js
var require_dist_node11 = __commonJS({
  "node_modules/octokit-plugin-create-pull-request/dist-node/index.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    function ownKeys(object, enumerableOnly) {
      var keys = Object.keys(object);
    ...
    var VERSION = "4.0.0";
    function createPullRequest2(octokit) {
      return {
        createPullRequest: composeCreatePullRequest.bind(null, octokit)
      };
    }
    createPullRequest2.VERSION = VERSION;
    exports.composeCreatePullRequest = composeCreatePullRequest;
    exports.createPullRequest = createPullRequest2;
  }
});

// node_modules/@putout/git-status-porcelain/lib/porcelain.js
var require_porcelain = __commonJS({
  "node_modules/@putout/git-status-porcelain/lib/porcelain.js"(exports, module2) {
    "use strict";
    var { spawnSync } = require("child_process");
    var isString = (a) => typeof a === "string";
    module2.exports = (str, filter = {}) => {
      if (!isString(str)) {
        filter = str || {};
        str = run();
      }
    ...
    module2.exports.getNames = getNames;
    ...
    var isModified = ({ mode }) => /M/.test(mode);
    var isAdded = ({ mode }) => /A/.test(mode);
    var isRenamed = ({ mode }) => /R/.test(mode);
    var isDeleted = ({ mode }) => /D/.test(mode);
    var isUntracked = ({ mode }) => /\?/.test(mode);
    var check = ({ added, modified, untracked, deleted, renamed }) => (file) => {
      let is = false;
      if (added)
        is = is || isAdded(file);
      if (modified)
...
var readActionConfig = () => {
  ...
  if (branchName) {
    (0, import_core.info)(`readActionConfig: branchName is specified as [${branchName}]`);
  } else {
    (0, import_core.info)(`readActionConfig: branchName is not specified, using default`);
  }
  const notifiedUsers = (0, import_core.getInput)("notified_users");
  if (notifiedUsers) {
    (0, import_core.info)(`readActionConfig: notifiedUsers are specified as [${notifiedUsers}]`);
  }
...
// src/index.ts
var import_octokit_plugin_create_pull_request = __toESM(require_dist_node11());
var import_fs = require("fs");
var porcelain = require_porcelain();
var checkCargoBinaries = async () => {
  const tryInstall = async (command, binary, version) => {
    try {
      let commandStdout = "";
      await (0, import_exec.exec)("cargo", [command, "--version"], {
        listeners: {
          stdout: (data) => commandStdout += data.toString()
        }
      });
      if (!commandStdout.includes(version)) {
        await (0, import_exec.exec)("cargo", ["install", "--locked", "--force", `${binary}@${version}`]);
      }
    } catch (e) {
      await (0, import_exec.exec)("cargo", ["install", "--locked", "--force", `${binary}@${version}`]);
    }
  ...
  await (0, import_exec.exec)("cargo", ["upgrade", "--version"]);
  await (0, import_exec.exec)("cargo", ["outdated", "--version"]);
};
var checkOutdated = async (pkg) => {
  let outdatedResult = "";
  const options = pkg ? ["outdated", "-p", pkg, "--format=json"] : ["outdated", "--format=json"];
  const outdatedExitcode = await (0, import_exec.exec)("cargo", options, {
    listeners: {
      stdout: (data) => outdatedResult += data.toString()
    }
  });
  if (outdatedExitcode !== 0) {
    throw new Error(`Action could not able to check if package is outdated.`);
  }
  const results = outdatedResult.split(import_os.EOL).filter((x) => x).map((x) => JSON.parse(x));
...
var runUpgrade = async (packages, upgradeAll, incompatible) => {
  ...
  const upgrade = async (pkg) => {
    ...
    await (0, import_exec.exec)("cargo", options);
  };
  if (upgradeAll) {
    (0, import_core2.info)(`Trying to upgrade all packages in the manifest`);
    if (await checkOutdated()) {
      await upgrade();
      (0, import_core2.info)(`Upgraded all packages in the manifest`);
    } else {
      (0, import_core2.info)(`All packages are up to date`);
      return false;
    }
  } else {
...
var buildPullRequest = async (ghToken, branchName, prTitle, notifiedUsers) => {
  ...
  if (existingPR) {
    (0, import_core2.info)(`Found existing PR #${existingPR.number} for branch ${branchName}, will try to update it`);
    const current = await octokit.rest.pulls.get({
      ...import_github.context.repo,
      pull_number: existingPR.number
    });
    if (current.data.commits > 1) {
      (0, import_core2.info)(`PR #${existingPR.number} has more than 1 commit, will not update it`);
      return;
    }
  } else {
    (0, import_core2.info)(`Trying to create a new PR for branch ${branchName}`);
  }
  const updatedFiles = porcelain({
    untracked: true,
    modified: true,
    added: true,
    deleted: true
  ...
  if (prResponse) {
    (0, import_core2.info)(`PR is available at ${prResponse.data.html_url}`);
  } else {
    (0, import_core2.error)(`Failed to create a PR`);
  }
};
var main = async () => {
